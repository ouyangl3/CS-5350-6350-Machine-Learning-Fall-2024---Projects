...
def calculate_error_rate(data, tree, attribute_indices, output_file="predictions.csv"):
    ...
    for idx, example in enumerate(data, start=1):  # 给每个示例分配一个唯一ID，从1开始
    ...
    with open(output_file, mode='w', newline='') as file:
        ...
        writer.writerow(["ID", "Prediction"])  # 写入头部
        writer.writerows(predictions)  # 写入每行的ID和预测结果
...
# print(decision_tree)
error = calculate_error_rate(train_data, decision_tree, attribute_indices)
